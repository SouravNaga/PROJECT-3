...
class DatabaseManage():
	...
	def __init__(self):
		...
		try:
			con = lite.connect(newPath+'\\studentt.db')
			with con:
				cur = con.cursor()
				cur.execute("CREATE TABLE IF NOT EXISTS Studentts(Id INTEGER PRIMARY KEY AUTOINCREMENT, Name TEXT, Class TEXT)")
	...
	def insertData(self,name,className):
		...
		try:
			with con:
				cur = con.cursor()
				cur.execute("INSERT INTO Studentts(Name, Class) VALUES('"+name+"','"+className+"')")
	...
	def delData(self,id):
		...
		try:
			with con:
				cur = con.cursor()
				sql = "DELETE FROM Studentts WHERE Id = ?"
				cur.execute(sql,[id])
		except Exception as e:
			print(str(e))
	...
	def fetchData(self, id):
		...
		try:
			with con:
				cur = con.cursor()
				sql = "SELECT * FROM Studentts WHERE Id = ?"
				cur.execute(sql,[id])
				rows = cur.fetchall()
				for row in rows:
					current = []
	...
	def updateData(self, id, name, className):
		...
		try:
			with con:
				cur = con.cursor()
				sql = "UPDATE Studentts SET Name = ?, Class = ? WHERE Id = ?"
